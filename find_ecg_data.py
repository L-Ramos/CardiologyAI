...
def plot_signal(signal,file_name,path_files,list_files=''):
    
    result = next((True for n in list_files if file_name in n), False)
    ...
    if not result:
        ...
        plt.savefig(r"\\amc.intra\users\L\laramos\home\Desktop\images\extra\\"+file_name+"_S_S_S.png")
        plt.close(fig) 
        #plt.show()
...
def fix_excel(df,excel_field,excel_surgery,label):
    """
    ...
    df[label]=df[label].replace(999,np.nan)
    
    l=df[label].dropna()
    
    excel_labels = list(l.astype(int))    
   
    excel_labels =dict(zip(excel_ids,excel_labels))
    
    return(excel_ids,excel_dates,excel_labels)
    ...
    for key in tqdm(filter_date): 
        ...
        for file in file_names:       
            with open(file) as fd:
                    ...
                    if (re.search('Sinusritme',rhythm,re.IGNORECASE) or re.search('Sinus ritme',rhythm,re.IGNORECASE) or 
                        re.search('Sinusbradycardie',rhythm,re.IGNORECASE) or re.search('Sinustachycardie',rhythm,re.IGNORECASE)):
                    #if ('Sinusritme'in rhythm or 'Sinus ritme'in rhythm or 'Sinusbradycardie'in rhythm or 'Sinustachycardie' in rhythm):
                    #if (re.search('Atriumfibrilleren',rhythm,re.IGNORECASE) or re.search('atrium fibrilleren',rhythm,re.IGNORECASE) or 
                    #    re.search('boezemfibrilleren',rhythm,re.IGNORECASE) or re.search('atrial fibrillation',rhythm,re.IGNORECASE)):
                        filter_rhythm[key].append(file)
                    else:
                        other_rhythms[key].append(file)
    return(filter_rhythm,other_rhythms)

#-------------------------------------------------------------------------------------------------------
# DON'T FORGET TO SET THE PATHs BELOW
#-------------------------------------------------------------------------------------------------------
...
excel_ids,excel_dates,excel_labels=fix_excel(df,excel_field,excel_surgery,label)
...
save_file(filter_date,path_files,'\\date_rhythm')
print("Done Fitering rhythm!") 


#Saving plots from the data
#for key in tqdm(filter_rhythm): 
#    signal=data_signal[key]    
#    for i in range(len(filter_rhythm[key])):
#        file_name=filter_rhythm[key][i]        
#        #plot_signal(np.array(signal[i*8]),os.path.basename(file_name),path_files,list_images)
#        plot_signal(np.array(signal[i*8]),os.path.basename(file_name),path_files)



#This will match image name from the images checked by the ECG experts and keep only the right rhythm ECG
list_images=glob.glob(r"\\amc.intra\users\L\laramos\home\Desktop\Cardio ECG\filtered_images\*.png")
image_dict=defaultdict(list)
for key in tqdm(filter_rhythm):
    names=filter_rhythm[key]
    for i in range(len(names)):
        name=os.path.basename(names[i])
        result = next((True for n in list_images if name in n), False)
        if result:
            image_dict[key].append(names[i])

#Saving the file            
save_file(image_dict,path_files,'\\date_checkedimages')

data_signal = defaultdict(list)  

#finally fixing it to a nice working format
for key in tqdm(image_dict): 
        file_names=image_dict[key]
        for i in range(len(file_names)): 
            with open(file_names[i]) as fd:
                ecg_dict = xmltodict.parse(fd.read(), process_namespaces=True)
            mean_wave, leads = ecg_dict['RestingECG']['Waveform']
...
#Saving the final data format, a dictionary with all the ecg leads and the corresponding labels
all_keys=list(excel_labels.keys())

final_data=list()
final_labels=list()
for key in all_keys:
    if key in data_signal.keys():
        final_data.append(data_signal[key])
        final_labels.append(excel_labels[key])
        
np.save(r"\\amc.intra\users\L\laramos\home\Desktop\Cardio ECG\final\data\final_data.npy",final_data)
np.save(r"\\amc.intra\users\L\laramos\home\Desktop\Cardio ECG\final\data\final_labels.npy",final_labels)
        



#FRom here on it is just some plotting, you can comment all this

total_neg=0   
total_pos=0 
for i,k in enumerate(final_labels):
    if k==1:
        total_pos+=len(final_data[i])
    else:
        total_neg+=len(final_data[i])
...
for key in (final_data):
    total_ecgs+=len(key)    
print("Total ECGs after filtering by hand-checked images:%d, total patients:%d "%(total_ecgs,len(final_data)))
